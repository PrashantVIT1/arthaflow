...
class ETLService:
    ...
    def _update_status(self, status: str, current_stage: str = None, progress: float = None, 
                      records_processed: int = None, total_records: int = None, error_message: str = None) -> None:
        """Update ETL status."""
    ...
    def _update_state(self, dataset_source: str = None, import_mode: str = None, 
                     uploaded_files: List[str] = None, upload_timestamp: datetime = None,
                     last_execution: dict = None, last_successful_import_timestamp: datetime = None) -> None:
    ...
    def run_pipeline(self, request: ETLRunRequest) -> ETLRunResponse:
        ...
        try:
            ...
            self._add_log("info", f"Starting ETL pipeline in {request.import_mode} mode", "initialization")
            ...
            if request.dataset_source == 'sample':
                try:
                    ...
                    self._add_log("info", "Clearing existing business data before sample import", "clear")
                    self._clear_tables(engine)
                    ...
                    self._load_sample_data(engine, response, "replace")  # Use replace mode for sample
                    ...
                    self._update_status("completed", "Completed", 1.0)
                    self._add_log("info", f"Sample dataset loaded successfully. {response.total_records_processed} records processed.", "completion")
                    ...
                    self._update_state(
                        ...
                        last_execution={
                            ...
                            "records_skipped": response.customers_skipped + response.products_skipped + response.orders_skipped,
                            "completed_at": datetime.now().isoformat()
                        },
                    ...
                    self._add_log("error", f"Sample dataset import failed: {str(e)}", "error")
                ...
                if request.dataset_source == 'custom':
                    if not request.files:
                        ...
                        self._add_log("error", "No files provided for custom dataset", "load")
                        response.message = "No files provided for custom dataset"
                        return response
                    self._update_status("running", "Loading custom data", 0.2)
                    self._add_log("info", f"Loading {len(request.files)} custom files", "load")
                    self._load_custom_data(engine, request.files, response, "replace")
                else:
                    self._update_status("error", error_message=f"Replace mode only supports custom dataset source")
                    self._add_log("error", f"Replace mode only supports custom dataset source", "initialization")
                    response.message = f"Replace mode only supports custom dataset source"
                    return response
                ...
                if request.dataset_source == 'sample':
                    ...
                    self._add_log("info", "Loading sample dataset in append mode", "load")
                    ...
                    if not request.files:
                        ...
                        return response
                    self._update_status("running", "Loading custom data", 0.2)
                    self._add_log("info", f"Loading {len(request.files)} custom files in append mode", "load")
                    self._load_custom_data(engine, request.files, response, "append")
                else:
                    self._update_status("error", error_message=f"Invalid dataset source: {request.dataset_source}")
                    self._add_log("error", f"Invalid dataset source: {request.dataset_source}", "initialization")
                    response.message = f"Invalid dataset source: {request.dataset_source}"
                    return response
            
            else:
                self._update_status("error", error_message=f"Invalid import mode: {request.import_mode}")
                self._add_log("error", f"Invalid import mode: {request.import_mode}", "initialization")
            ...
            self._update_status("completed", "Completed", 1.0)
            self._add_log("info", f"ETL pipeline completed. {response.total_records_processed} records processed.", "completion")
            ...
            self._update_state(
                dataset_source=request.dataset_source,
                import_mode=request.import_mode,
                uploaded_files=[f["original_name"] for f in request.files] if request.files else None,
                last_execution={
                    "operation": operation_name,
                    "execution_time": response.execution_time,
                    "customers_imported": response.customers_inserted,
    ...
    def _insert_with_duplicate_check(self, engine, df: pd.DataFrame, table_name: str, pk_column: str) -> tuple[int, int]:
        """
        ...
        if skipped > 0:
            self._add_log("info", f"Skipped {skipped} duplicate records in {table_name}", "load")
        
        return inserted, skipped
    
    def _load_custom_data(self, engine, files: list, response: ETLRunResponse, import_mode: str) -> None:
        ...
        for file_info in sorted_files:
            ...
            if table_name == 'orders' and 'total_amount' not in df.columns and 'quantity' in df.columns and 'unit_price' in df.columns:
                df['total_amount'] = df['quantity'] * df['unit_price']
